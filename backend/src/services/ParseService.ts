...
function matchEntryType(sentence: string) {
...
function matchPriority(sentence: string) {
...
function matchProject(sentence: string, knownProjects: Array<{ id?: string; name?: string }>) {
...
function matchPeople(sentence: string, knownPeople: string[]) {
...
function parseTags(sentence: string) {
  return Array.from(sentence.matchAll(/#([\w-]+)/g)).map((m) => m[1]);
}

function parseRefs(sentence: string) {
...
function resolveRelativeDate(now: string, keyword: string) {
  const base = dayjs.tz(now, CONFIG.timezone);
...
function parseTime(sentence: string, now: string) {
  const lowered = sentence.toLowerCase();
  ...
  return {
    mentioned_range: {
      start: dayjs.tz(start, CONFIG.timezone).toISOString(),
      end: dayjs.tz(end, CONFIG.timezone).toISOString()
    },
    duration_minutes: null
  };
}

function parseDue(sentence: string, now: string) {
  const lowered = sentence.toLowerCase();
  if (lowered.includes("mai") || lowered.includes("ngày mai") || lowered.includes("tomorrow")) {
    const dueAt = dayjs.tz(now, CONFIG.timezone).add(1, "day").hour(9).minute(0).second(0);
...
function buildFollowups(sentence: string, now: string) {
  const lowered = sentence.toLowerCase();
...
export class ParseService {
  parseTranscript({
    ...
    now: string;
    transcript: string;
    known_projects: Array<{ id?: string; name?: string }>;
    known_people: string[];
  }) {
  ...
  summarizeEod({ date, items }: { date: string; items: Array<{ entry_type: string; project?: { name?: string }; title: string; followups?: Array<{ title: string }>; priority?: number }> }) {
  ...
  smartInbox({
    ...
    item: { title?: string };
    known_projects: Array<{ id?: string; name?: string }>;
    recent_items: Array<{ title?: string }>;
  }) {
